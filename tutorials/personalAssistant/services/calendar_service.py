...
from services.notion_service import add_calendar_event_to_notion, add_generated_content_to_notion
from agentic.contentWriter import ContentWritingAgent
...
async def process_calendar():
    ...
    for event in calendar_data['today'] + calendar_data['tomorrow']:
        ...
        if any(keyword in event_summary for keyword in ["post", "article", "blog", "linkedin", "medium", "content"]):
            ...
            add_calendar_event_to_notion(parsed_event)  # Save modified event in the calendar table

            # Save the generated content to the new content table
            generated_content_entry = GeneratedContent(
                title=event['summary'],
                content=generated_content,
                date=format_date(start_date)
            )
            add_generated_content_to_notion(generated_content_entry)
